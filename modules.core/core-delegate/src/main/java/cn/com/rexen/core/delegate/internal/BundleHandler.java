...
import cn.com.rexen.core.delegate.Constants;
...
public class BundleHandler {
    ...
    private static final Logger LOGGER = Logger.getLogger(BundleHandler.class);
    ...
    public void handle() {
        LOGGER.info(String.format("injecting ClassProvider-Service into bundle {0}.", bundle.getSymbolicName()));
    ...
    private Set<String> getMatchingResources(Collection<String> fileFilters) {
        ...
        for (String p : fileFilters) {
            ...
            if (resources == null) {
                LOGGER.warn(String.format("no resources found for pattern {0}", p));
                continue;
            }
    ...
    private static Set<String> discoverClasses(Bundle bundle) {
        ...
        if (classEntries == null) {
            LOGGER.info(String.format("bundle {0} doesn't contain class files", bundle));
