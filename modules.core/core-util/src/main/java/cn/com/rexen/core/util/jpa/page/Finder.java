...
public class Finder {
    ...
    public Finder setParam(int param, Object value) {
        getIparams().add(param);
        getValues().add(value);
    ...
    public Query setParamsToQuery(Query query) {
        ...
        if (iparams != null) {
            for (int i = 0; i < iparams.size(); i++) {
                query.setParameter(iparams.get(i), values.get(i));
            }
        }
    ...
    public List<Integer> getIparams() {
        if (iparams == null) {
            iparams = new ArrayList<Integer>();
        }
        return iparams;
    }
